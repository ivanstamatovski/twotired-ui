...
- **WebView geolocation WORKS (verified on emulator 2026-07-04).** The app uses browser `navigator.geolocation.watchPosition`, NOT `@capacitor/geolocation`, and the earlier worry that Capacitor's Android bridge wouldn't grant it was unfounded — on an API-35 emulator the rider got a real fix (map centered on the emulated location) with `ACCESS_FINE/COARSE_LOCATION` `granted=true` and no geolocation errors in logcat. Capacitor 8's `BridgeWebChromeClient` auto-grants WebView geolocation when the app holds the native location permission. **Still to confirm on a real device:** the first-run runtime permission *prompt* (the emulator appears to pre-grant, so the prompt UX wasn't exercised).
